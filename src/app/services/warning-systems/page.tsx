        ...
        <div className="max-w-7xl mx-auto px-4 grid lg:grid-cols-2 gap-10 lg:gap-16 items-center">
          <div className="space-y-6 lg:space-y-8 text-center lg:text-left">
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white border border-slate-200 text-slate-700 font-bold text-xs lg:text-sm tracking-wide shadow-sm">
              <ShieldAlert className="w-4 h-4 text-amber-600" /> Modüler İkaz Donanımları
            </div>
            ...
            <h1 className="text-3xl sm:text-5xl lg:text-7xl font-extrabold leading-tight tracking-tight text-slate-900">
              ...
              <span className="text-amber-600">Sinyalizasyon Çözümleri</span>
            </h1>
            ...
            <div className="flex flex-wrap justify-center lg:justify-start gap-4">
              <Link
                href="/contact"
                className="bg-amber-600 text-white px-6 py-3 lg:px-8 lg:py-4 rounded-xl font-bold hover:bg-amber-700 transition-all hover:scale-105 shadow-xl shadow-amber-500/20 flex items-center gap-2 text-sm lg:text-base"
              >
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">
          {[
            ...
            },
            {
              ...
              desc: "Araç ve ekipman trafiğini yönetmek için otomasyon sistemlerine entegre edilebilen, dayanıklı ve yüksek performanslı ışık birimleri.",
              icon: Activity,
              color: "amber"
            }
          ].map((item, idx) => (
          ...
          <div className="grid lg:grid-cols-2 gap-16 items-center">
            <div className="space-y-6">
              ...
              <p className="text-lg text-slate-600 leading-relaxed font-light">
                Çözümlerimiz yalnızca görsel ve sesli uyarı üretmekle kalmaz, aynı zamanda sahada farkındalık oluşturarak kontrollü ve güvenli hareket akışına katkı sağlar. Hareketli ekipman geçişleri, yaya yolları, yükleme alanları ve kritik risk bölgelerinde; sesli anons, ışıklı yönlendirme ve projeksiyon tabanlı uyarılar birlikte kullanılarak güçlü bir sinyalizasyon altyapısı oluşturulur.
              </p>
              <p className="text-lg text-slate-600 leading-relaxed font-light">
                ...
                <div className="aspect-[4/3] rounded-[2.5rem] overflow-hidden shadow-2xl relative">
                  ...
                  <div className="absolute bottom-6 right-6 flex gap-2 z-20">
                    {sliderImages.map((_, idx) => (
                      <div
                        key={idx}
                        className={`w-2 h-2 rounded-full transition-all duration-300 ${activeImage === idx ? 'bg-amber-600 w-6' : 'bg-white/50'}`}
                      />
                    ))}
                  </div>
                </div>
